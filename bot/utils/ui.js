const { ensureSession, getCurrentOpId } = require("./sessionState");
...
function logCommandError(ctx, error) {
  const opId = getCurrentOpId(ctx) || "unknown";
  const command =
  ...
  const requestId = error?.apiContext?.requestId || null;
  ...
  console.error(
    `[${opId}] Command error [${command}] [requestId=${requestId}] for user ${username} (${userId}):`,
    message,
  );
}
...
async function styledAlert(ctx, message, options = {}) {
  return ctx.reply(section("⚠️ Notice", [message]), {
    parse_mode: "Markdown",
    ...options,
  });
}
