...
bot.on("callback_query:data", async (ctx) => {
  ...
  try {
    ...
    if (
      ...
      if (isConversationAction && ctx.session?.currentOp?.id && recoveryTarget) {
        await recoverConversationFromCallback(
          ctx,
          action,
          recoveryTarget.conversationTarget,
          { cancelActiveFlow, resetSession, clearMenuMessages },
        );
        finishMetric("recovered", { reason: "menu_expired" });
        return;
      }
      await safeAnswerCallbackQuery(ctx, {
    ...
    if (
      ...
      if (isConversationAction && ctx.session?.currentOp?.id && recoveryTarget) {
        ...
        finishMetric("recovered", { reason: "stale_menu_message" });
        return;
      }
      await safeAnswerCallbackQuery(ctx, {
    ...
    if (recoveryTarget) {
      ...
      // Inline buttons generated by an active conversation can occasionally leak to this
      // global router on double taps/retries. Recover the conversation instead of
      // silently deferring, so users never get stuck in a dead-end state.
      if (
        hasActiveOp &&
        callbackOpToken &&
        currentOpToken &&
        ...
        finishMetric("recovered", { reason: "active_op_router_leak" });
        return;
      }
