...
export function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (isAdmin) {
      void apiFetch<{
        ok: boolean;
        tasks: { id: number; run_at: string; number: string }[];
      }>("/webapp/callbacks?limit=3")
        .then(
          (response: {
            tasks?: { id: number; run_at: string; number: string }[];
          }) => setCallbackTasks(response.tasks || []),
        )
        .catch(() => {});
    }
  ...
  const stats = useMemo(() => {
    ...
    const active = calls.filter((call) =>
      ["in-progress", "answered", "ringing"].includes(String(call.status)),
    ).length;
    const completed = calls.filter(
      (call) => String(call.status) === "completed",
    ).length;
    ...
    <div className="wallet-page">
      <div className="hero-card">
        <div className="hero-header">
          ...
          <div
            className={`hero-status ${heroStatus === "Live" ? "live" : "idle"}`}
          >
            {heroStatus}
          </div>
        </div>
      ...
      <div className="pill-row">
        ...
        >
          <span className="pill-icon" aria-hidden="true">
            <svg viewBox="0 0 24 24">
              <path
                d="M7 17a2 2 0 0 1-2-2V9a2 2 0 0 1 2-2h4.5"
                fill="none"
                stroke="currentColor"
                strokeWidth="1.6"
                strokeLinecap="round"
        ...
        >
          <span className="pill-icon" aria-hidden="true">
            <svg viewBox="0 0 24 24">
              <path
                d="M4 7h16l-2.4 9.6a2 2 0 0 1-2 1.4H8.4a2 2 0 0 1-2-1.4L4 7z"
                fill="none"
                stroke="currentColor"
                strokeWidth="1.6"
                strokeLinejoin="round"
      ...
      <div className="banner-card">
        <div className="banner-content">
          ...
          <div className="banner-subtitle">
            Answer in the Mini App to connect incoming calls instantly.
          </div>
        </div>
          ...
          <div className="card-header">
            ...
            <span className="card-header-muted">
              {callbackTasks.length
                ? `${callbackTasks.length} scheduled`
                : "None"}
            </span>
          </div>
          {callbackTasks.length === 0 ? (
            <div className="empty-card">
              ...
              <div className="empty-subtitle">
                Callback tasks will show up here.
              </div>
            </div>
          ) : (
            <div className="card-list">
